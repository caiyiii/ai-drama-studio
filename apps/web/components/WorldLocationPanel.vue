  ...
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <h2 class="mt-1 font-display text-3xl">地点</h2>
      </div>
    ...
    <p v-if="items.length === 0" class="text-sm text-zinc-500">还没有地点。记录星球、城市、秘境或战场。</p>
    ...
    <AppModal :open="showForm" :title="editing ? '编辑地点' : '新增地点'" description="这个地点属于哪个文明，发生过什么。" @close="showForm = false">
...
import { WORLD_LOCATION_TYPES } from "@ai-drama-studio/config";
import type { Civilization, CreateWorldLocationInput, WorldLocation } from "@ai-drama-studio/types";
...
const emit = defineEmits<{
  create: [data: CreateWorldLocationInput];
  update: [id: string, data: CreateWorldLocationInput];
  remove: [id: string];
}>();

const typeOptions = WORLD_LOCATION_TYPES.map((item) => ({ value: item, label: item }));
const civilizationOptions = computed(() => [
  { value: "", label: "无所属文明" },
  ...props.civilizations.map((item) => ({ value: item.id, label: item.name })),
]);
const showForm = ref(false);
const editing = ref<WorldLocation | null>(null);
const pending = ref<WorldLocation | null>(null);
const form = reactive({
  name: "",
  type: "其他",
  civilizationId: "",
...
function openEdit(item: WorldLocation) {
  editing.value = item;
  form.name = item.name;
...
function onSubmit() {
  const data: CreateWorldLocationInput = {
    name: form.name.trim(),
    type: form.type,
